...
{
    /*FIXME: write this*/
	if(((struct Task*)left)->priority < ((struct Task*)right)->priority){
		return -1;
	}
	else if(((struct Task *)left)->priority > ((struct Task*)right)->priority){
		return 1;
	}
...
{
  /*FIXME: Write this */
	struct Task* newt = malloc(sizeof(struct Task));
	strcpy(newt -> description,desc);
