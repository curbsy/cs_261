...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
  addDynArr(heap, val);
  int y = sizeDynArr(heap)-1;
  
  
  while(y){
	  if((*compare)(getDynArr(heap,y), getDynArr(heap,(y-1)/2)) == -1){
		  swapDynArr(heap,y,(y-1)/2);
		  y= (y-1)/2;	  
	  }
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  ...
  if(right < max){  //for 2 children
	  ...
	  else if(left < max){  //for 1 child
		  if((*compare)(getDynArr(heap,pos), getDynArr(heap,left)) ==1){
...
/*  int end = sizeDynArr(heap)-1;
  putDynArr(heap,0,getDynArr(heap,end));
  removeAtDynArr(heap,end);
  _adjustHeap(heap,end,0,compare);*/
  heap->size--;
...
//  if((*compare)(heap->size-1, heap->size-2) ==-1){
//  swapDynArr(heap,heap->size-1, heap->size-2);
//	  }
